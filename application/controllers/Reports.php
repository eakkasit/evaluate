...
class Reports extends CI_Controller
{
	...
	{
		redirect(base_url("reports/dashboard_reports"));
		exit;
	}

	public function dashboard_reports($year = null)
	{
		...
		$data['content_data'] = array(
			'search_url' => base_url("reports/dashboard_reports"),
		...
		$data['content_view'] = 'pages/dashboard_reports';
		$this->load->view($this->theme, $data);
	}

	public function view_report($meeting_id = null)
	{
		$data['content_data'] = array(
			'prefix_list' => $this->Commons_model->getPrefixList(),
			'status_list' => $this->Commons_model->getDataStatusList(),
		...
		$data['content_view'] = 'pages/view_report';
		$this->load->view($this->theme, $data);
	}

	public function export($meeting_id = null, $type = '')
	{
		$data = array(
			'prefix_list' => $this->Commons_model->getPrefixList(),
			'status_list' => $this->Commons_model->getDataStatusList(),
		...
		if($type == 'pdf'){
			$pdfFilePath = "รายงานการประชุม{$data['meeting_data']->meeting_name}.pdf";
			$html = $this->load->view('pages/report_export_pdf', $data,true);
			$this->m_pdf->pdf->WriteHTML($html);
			$this->m_pdf->pdf->Output($pdfFilePath, 'D');
			exit;
		}else if($type == 'word'){
			$this->load->view('pages/report_export_word', $data);
		}else if($type == 'excel'){
			$this->load->view('pages/report_export_excel', $data);
		}else{
			redirect(base_url("reports/view_report"));
			exit;
		}
	}
}
